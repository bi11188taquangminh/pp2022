...
class Student:
    def __init__(self):
        ...
        self.dob = ()
        self.list_courses = []
#list_courses includes (id,name,mark(default -1),credit(default 0))
    ...
    def add_courses (self,x,y):
        self.list_courses.append((x.lower(),y.lower(),-1,0))
    ...
    def set_mark(self, course_id, mark):
        ...
        for course in self.list_courses:
            if course[0] == course_id:
                self.list_courses.append((course[0],course[1],math.floor(x),course[3]))
                self.list_courses.remove(course)

    def set_course_credit(self, course_id, credit):
        x=int(credit)
        for course in self.list_courses:
            if course[0] == course_id:
                self.list_courses.append((course[0],course[1],course[2],x))
                self.list_courses.remove(course)
                

    def display_student_courses_list(self):
        print("mark = -1 means no information yet\n")
        print("credit = 0 means no information yet\n")
        print ("this student (course,id,mark,credits) are: ")
    ...
    for student in list_student:
        if student.get_id() == x:
            print (f"this student name is {student.get_name()} with \
                id: {student.get_id()}\n")
            print (f"the date of birth is: \
                {student.get_dob()[0]}/{student.get_dob()[1]}/{student.get_dob()[2]}\n")
...
while True:
    print("Choose one option by typing:\n" \
                ...
                "\t8. Close program\n"
                "\t9. Add course credit\n"
                "\t10. Display average gpa of that student\n"
                "Input: " )
    choice = int(input())
    if choice == 8:
        break
        ...
        y = search_student(x)
        m = input("enter the precise course id please\t")
        z = input("please enter the number of credits of that subject\t")
        y.set_course_credit(m,z)

    elif choice == 10:
        x = input("Please type the precise student id\t")
        y = search_student(x)
        z = y.calc_student_gpa()
        print (f"this student have the gpa of: {z}")